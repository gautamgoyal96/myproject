    ...
    <tbody>
        ...
        <tr>
         ...
         <td><?php echo !empty($get->address) ? wordwrap($get->address, 40, "<br />\n") : "NA"; ?></td>
            ...
            <td class="action">
                <?php if($get->status == 1) : ?>
                <a href="<?php echo base_url()."user/activeCustomers/" . $get->id.'/'.$pag; ?>"  class="btn btn-sm btn-icon btn-warning btn-outline btn-round" title="Inactive"><i class="icon md-close" aria-hidden="true"></i> </a>
                <?php  else : ?>
                <a href="<?php echo base_url()."user/activeCustomers/" . $get->id.'/'.$pag; ?>" class="btn btn-sm btn-icon btn-success btn-outline btn-round"  title="Active"><i class="icon md-check"></i> </a>
                <?php endif; ?>
               
              <!--   <a data-href="<?php echo base_url().'user/deleteCustomers/'.$get->id.'/'.$pag;?>" class="btn btn-sm btn-icon btn-danger btn-outline btn-round delete"
